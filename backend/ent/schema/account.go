...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		...
		// owner_user_id: 托管账号的归属用户（NULL = 平台/管理员账号）。
		// 用于「账号托管市场」：非管理员用户把自己的订阅号托管进共享池后，
		// 此字段记录号主，供用量归属与收益结算使用。调度器不读取该字段。
		field.Int64("owner_user_id").
			Optional().
			Nillable().
			Comment("Owner user id for a user-hosted account (NULL = platform/admin account)."),
...
func (Account) Edges() []ent.Edge {
	return []ent.Edge{
		...
		// owner: 托管账号的号主（可选）。使用 owner_user_id 外键字段；
		// 平台/管理员账号此边为空。反向边为 User.hosted_accounts。
...
func (Account) Indexes() []ent.Index {
	return []ent.Index{
		...
		index.Fields("owner_user_id"), // 按号主筛选托管账号
	}
}
